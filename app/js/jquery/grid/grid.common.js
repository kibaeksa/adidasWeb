...
var createModal = function(aIDs, content, p, insertSelector, posSelector, appendsel) {
	...
	jQuery(mw).addClass("ui-widget ui-widget-content ui-corner-all ui-jqdialog").attr("style","border: 3px solid #696969;");
	mw.id = aIDs.themodal; 
	...
	jQuery(mc).addClass("ui-jqdialog-content ui-widget-content").attr("id",aIDs.modalcontent).attr("style","background-color:#FFFFFF;");
	...
	if(edtrul) {
		...
		if(edtrul.number === true) {
			if( !(rqfield === false && isEmpty(val)) ) {
				...
				if(isNaN(val)) return [false," : "+jQuery.jgrid.edit.msg.number,""];
			}
		}
