...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		/** class ui-widget-content1 => ui-widget-content1로 변경 20100322*/
		var eg = $("<div class='ui-jqgrid ui-widget ui-widget-content1 ui-corner-all'></div>");
		 ...
		 * 
		 * 2010-04-13 김중태
		 * 그룹이름이 있을 경우 에러가 나는 것을 방지하기 위한 코드추가
		 */
		formatCol = function (pos, rowInd){
			
			if(ts.p.colGroupNames.length > 0) {
				/*var grpArr = ts.p.colGroupNames;
				var grpColNo = 1;
				
				for(var i1=0; i1<grpArr.length; i1++) {
					var grpStr = grpArr[i1];
					var arr = grpStr.split(":");
					
					var grpCol = Number(arr[1]);
					var grpNum = Number(arr[2]);
					
					if(grpCol == pos) {
						grpColNo = grpCol;
					} else (grpCol < pos && (grpCol + grpNum) > pos) {
						grpColNo = 0;
					}
				}*/
		...
		addRowNum = function (pos,irow,pG,rN) {
			...
			prp = formatCol(pos,irow);
			/** 20100316 ui-state-default -> ui-state-default-num 으로 변경 */
		 ...
		 * 
		 * 2010-04-13 김중태
		 * 그룹이름이 있을 경우 rowspan 과 colspan 옵션을 주는 코드 추가
		 */
		if(this.p.colGroupNames.length > 0) {
			if(this.p.colGroupNames.length <= 0) {
				alert("ㅡ.ㅡ;;;;");
			} else {
				var overRow = "";
				...
				for(i=0;i<this.p.colNames.length;i++){
					//overRow += "<th role='columnheader' class='ui-state-default1 ui-th-column ui-th-"+dir+"'><div "+tdc+"></div></th>";
					/** class ui-state-default => ui-state-default1로 변경 20100322*/
					/*overRow += "<th role='columnheader' class='ui-state-default1 ui-th-column ui-th-"+dir+"'>";
					idn = ts.p.colModel[i].index || ts.p.colModel[i].name;
					overRow += "<div id='jqgh_"+ts.p.colModel[i].name+"' "+tdc+">"+ts.p.colNames[i];
					if (idn == ts.p.sortname) {
						ts.p.lastsort = i;
					} 
					overRow += imgs+"</div></th>";*/
					if(chkNum == 0) {
						for(var j=0;j<this.p.colGroupNames.length;j++) {
							var str = this.p.colGroupNames[j].split(":");
							
							if(str[1] > i)
								break;
							if(str[1] == i) {
								middleRow += "<th colspan='" + str[2] + "' role='columnheader' class='ui-state-default1 ui-th-column ui-th-"+dir+"' >";
								
								middleRow += "<div "+tdc+">" + str[0];
								middleRow += "</div></th>";
								chkNum = Number(str[2]);
								break;
							}
						...
						if(chkNum == 0) {
							if( !ts.p.colModel[i].hidden ) {
								/** class ui-state-default => ui-state-default1로 변경 20100322*/
								middleRow += "<th rowspan='3' role='columnheader' class='ui-state-default1 ui-th-column ui-th-"+dir+"'";
								
								/** 헤더 그룹 머지일때 올리브 색 지정 2011.04.25 최병언*/
								if (ts.p.colModel[i].editable == true){
									middleRow += " style='background-color:#808000;'";
								}
								
								middleRow += ">";
								
								idn = ts.p.colModel[i].index || ts.p.colModel[i].name;
								middleRow += "<div id='jqgh_"+ts.p.colModel[i].name+"' "+tdc+" >"+ts.p.colNames[i];
								if (idn == ts.p.sortname) {
									ts.p.lastsort = i;
								} 
								middleRow += imgs+"</div></th>";
							}
							
							overRow += "<th style='height:0px;'></th>";
							//lineRow += "<th style='height:1px; bachground-color:#666666'></th>";
						}
					}
					if(chkNum > 0) {
						/** class ui-state-default => ui-state-default1로 변경 20100322*/
						underRow += "<th role='columnheader' class='ui-state-default1 ui-th-column ui-th-"+dir+"'";
						
						if (ts.p.colModel[i].editable == true){
							underRow += " style='background-color:#808000;'";
						}
						
						underRow += ">";
						
						idn = ts.p.colModel[i].index || ts.p.colModel[i].name;
						underRow += "<div id='jqgh_"+ts.p.colModel[i].name+"' "+tdc+">"+ts.p.colNames[i];
						if (idn == ts.p.sortname) {
							ts.p.lastsort = i;
						} 
						underRow += imgs+"</div></th>";
						
						overRow += "<th style='height:0px;";
						
						if (ts.p.colModel[i].editable == true){
							overRow += "background-color:#808000;";
						}
						overRow += "'></th>";
						lineRow += "<td style='height:1px;' bgcolor='#DDDDDD'></td>";
						
						chkNum--;
					}
				}
				//thead += overRow + "</tr><tr>" + middleRow + "</tr><tr>" + underRow;
				thead += overRow + "</tr><tr>" + middleRow + "</tr><tr>" + lineRow + "</tr><tr>" + underRow;
			}
		} else {
			for(i=0;i<this.p.colNames.length;i++){
				/** class ui-state-default => ui-state-default1로 변경 20100322*/
				thead += "<th role='columnheader' class='ui-state-default1 ui-th-column ui-th-"+dir + "'";
				
				if (ts.p.colModel[i].editable == true){
					thead += "style='background-color:#808000;'";
				}
				thead += ">";
				idn = ts.p.colModel[i].index || ts.p.colModel[i].name;
...
$.jgrid.extend({
	 ...
	 * 20100402 김중태
	 */
	setRowSelected : function(rowid, selected, cssp) {
		var nm, success=false;
		this.each(function(){
			var t = this, vl, ind, cp = typeof cssp;;
			if(!t.grid) {return false;}
			...
			switch(selected) {
				case "I" : 
					/**$(this.p.colModel).each(function(i){
						nm = this.name;
						if(t.p.treeGrid===true && nm == t.p.ExpandColumn) {
							$("td:eq("+i+") > span:first",ind).addClass("ui-state-selected-add");
						} else {
							$("td:eq("+i+")",ind).addClass("ui-state-selected-add");
						}
						success = true;
					});*/
					$(ind).addClass("ui-state-selected-add");
					break;
				case "U" : 
					$(ind).addClass("ui-state-selected-modify");
					break;
				case "D" : 
					$(ind).addClass("ui-state-selected-remove");
					break;
				case "S" : 
					$(ind).addClass("ui-state-selected-select");
					break;
				case "SN" : 
					$(ind).removeClass("ui-state-selected-select");
					break;
				case "N" : 
					$(ind).removeClass("ui-state-selected-add");
					$(ind).removeClass("ui-state-selected-modify");
					$(ind).removeClass("ui-state-selected-remove");
					$(ind).removeClass("ui-state-selected-select");
					break;
			}
			if(cp === 'string') {$(ind).addClass(cssp);} else if(cp === 'object') {$(ind).css(cssp);}
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			if(shrink===true) {
				...
				aw = nwidth-brd*vc-gw;
				if(!isNaN($t.p.height)) {
					/** 20100329 메뉴가 길어지면 좌측에 스크롤이 생기고, 그대 따라 하단에도 스크롤이 생기므로 그것을 방지하기 위해 */
