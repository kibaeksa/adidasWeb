...
front.use('/',express.static(path.join(__dirname , staticPath)));
front.use(bodyParser.json({limit: '50mb'}));
front.use(bodyParser.urlencoded({limit: '50mb', extended: true, parameterLimit:50000}));
...
front.get('/adidasCodingList',function(req , res){
    console.log(brand);
    brand = 'adidas';
...
front.get('/reebokCodingList',function(req , res){
    console.log(brand);
    brand = 'reebok';
...
front.post('/modifyHtmlJson',function(req , res){
